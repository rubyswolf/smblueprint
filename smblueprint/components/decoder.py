...
class decoder:
    def __init__(self, bp, bits, x=0, y=0, z=0):
        self.input = [sm.LogicGate(x, y, z, sm.LogicMode.OR) for i in range(bits)]
        for i in range(bits):
            bp.add_gate(self.input[i])
        inverted = [sm.LogicGate(x, y, z, sm.LogicMode.NOR) for i in range(bits)]
        for i in range(bits):
            bp.add_gate(inverted[i])
            self.input[i].connect_to(inverted[i])
        values = 2**bits
        self.output = [sm.LogicGate(x, y, z, sm.LogicMode.AND) for i in range(values)]
